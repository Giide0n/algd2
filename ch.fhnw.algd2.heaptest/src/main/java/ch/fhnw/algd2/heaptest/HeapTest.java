...
public class HeapTest<K> extends JFrame {
	...
	HeapTest(PriorityQueue<K> q) {
		...
		queueModel = q;
		heapView = new TreeComponent<>(queueModel);
		...
		text.addActionListener(e -> {
            try {
                queueModel.add(null, Long.parseLong(text.getText()));
                heapView.repaint();
            }
            catch (Exception ex) {
                ex.printStackTrace();
	...
	public static void main(String[] args) throws Exception {
		automaticTests();
		// consoleTest();
		Frame f = new HeapTest<>(new Heap<>(MAX_HEAP_SIZE));
		f.setSize(400, 300);
		f.setVisible(true);
	}
	...
	private static void consoleTest() {
		...
		long prioIn = 0;
		Heap<Long> heap = new Heap<>(MAX_HEAP_SIZE);
		int cnt = 0;
		...
		do {
			try {
				...
				System.err.println("Warnung: Eingabe ist nicht vom Typ 'long'" + "und wird deshalb ignoriert (" + e + ").");
			}
			catch (QueueFullException e) {
				System.err.println("Warnung: Die Queue ist bereits voll. Die letzte Eingabe wurde ignoriert.");
			}
		}
		while (!str.isEmpty() && cnt < MAX_HEAP_SIZE);
		// Heap als Array mit Prioritäten ausgeben zur Kontrolle auf stdout
		...
		System.out.println();
		if (!ok) System.out.println("Die Ordnungseigenschaft ist nicht erfuellt!!!");
	}

	private static void automaticTests() throws Exception {
		try {
			Heap<String> h = new Heap<>(2);
			h.add("1", 1);
			h.add("0", 0);
			if (2 != h.size()) {
	...
	private static class TreeComponent<K> extends JComponent {
		...
		private void drawTree(Graphics g, long[] q, int index, int x, int y, int dx, int dy, int c) {
